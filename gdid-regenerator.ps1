...
$userDevicePath = "$identityPath\DeviceIdentities\production\$([System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value)"

Remove-Item $systemDevicePath -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item $userDevicePath -Recurse -Force -ErrorAction SilentlyContinue
